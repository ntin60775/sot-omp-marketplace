...
```bash
# прочитать отчёт, убедиться, что расхождения — старые генерации пакета, и принять решение:
python3 scripts/consumers.py migrate --apply --accept-divergent            # снять и расходящиеся
python3 scripts/consumers.py migrate --apply --accept-unique               # снять и отсутствующие в пакетах
```

Решения раздельные: «старая версия того же файла» и «файла в пакете нет вовсе» — разные
случаи. `--keep` сильнее любого `--accept*`: названное остаётся.
...
| дрейф `legacy` держится | плоские копии на месте | `migrate` (отчёт) → `migrate --apply`; расходящиеся файлы требуют решения (`--accept-divergent`/`--accept-unique` — осознанно) |
