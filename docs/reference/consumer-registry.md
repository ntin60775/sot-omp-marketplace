...
Покрытие `gitignore`: шаблон покрывает требуемую строку, если совпадает с ней или
является её каталогом-префиксом (`.omp/` покрывает `.omp/plugins/` и `.omp/.backup-*`).
Ведущий `/` (привязка к корню) и хвостовой слэш на смысл не влияют — git читает
`.omp/plugins`, `/.omp/plugins/` и `.omp/plugins/` одинаково; `.gitignore` из одного `*`
покрывает всё. Отмена (`!`) покрытие снимает: она возвращает путь в git так же, как
отсутствие строки. Набор требуемых строк — канонический блок
