---
node_type: reference
title: Реестр потребителей каталога — формат, семантика, контракт инструмента
service: _platform
status: active
updated: 2026-09-16
links:
  documents: [../../.consumers.json, ../../schemas/consumer-registry.schema.json, ../../scripts/consumers.py]
  depends_on: [../decisions/plugin-delivery.md, marketplace-catalog.md]
  relates_to: [../ops/release-ontoship.md, ../plans/consumer-delivery.md]
---

# Реестр потребителей

## Зачем он есть

Поставка плагином решает «как обновлять», но не отвечает на «кого обновлять и что
у него стоит». Скан машины 2026-09-14 показал цену этого пробела: девять проектов с
`.omp/`, из них три с плагином, шесть на плоских копиях, а `erp-main` — плагин поверх
копий; плагина `1c` нет в пяти проектах; версии `unica` разъехались (0.12.0 против
0.12.3). Ни одна из этих величин не была видна ниоткуда: `omp plugin list` смотрит
только на текущий проект, а «список потребителей» не существовал.

Реестр — ответ: **один файл, который знает всех потребителей машины**, и инструмент,
который по нему считает дрейф и обновляет.

## Где живёт и почему так

| Что | Где | В git |
|---|---|---|
| Данные — `.consumers.json` в корне клона каталога | рядом с `.omp-plugin/marketplace.json` | **нет** (`.gitignore`) |
| Схема — `schemas/consumer-registry.schema.json` | `schemas/` этого репо | да |
| Инструмент — `scripts/consumers.py` | `scripts/` этого репо | да (создаётся планом [consumer-delivery](../plans/consumer-delivery.md)) |

Данные не коммитятся, потому что в них абсолютные пути и состав конкретной машины:
у второй машины тот же реестр выглядел бы иначе. Схема коммитится, потому что это
знание: формат обязан быть версионируемым и проверяемым, а не «как получилось у меня».

**Отвергнутые альтернативы** (обе пробовались как проект решения):

- *Реестр в машинном слое `sot-omp-core`* (`~/.omp/agent/…`). Машинный слой — верное
  место для машинного состояния, но инструмент уехал бы от каталога, который читает, и
  перестал бы быть частью клона каталога; синхронизацию пришлось бы вести вторым
  механизмом.
- *Плоский список вида `~/ontoship-projects.txt`* (архивный план `marketplace-delivery`).
  Список имён не несёт наблюдаемого состояния и не покрывает плагины чужих каталогов
  (`unica@unica`) — то есть ровно ту часть контура, где версии и разъезжаются.

## Реестр на новой машине

Реестр **не переносится** между машинами и не синхронизируется: в нём абсолютные пути
и состав конкретной машины. Поэтому он не восстанавливается копированием — он
**пересобирается наблюдением**:

```bash
python3 scripts/consumers.py init --root <каталоги-корни>   # машинный якорь: каталог + корни поиска
python3 scripts/consumers.py discover                       # кто есть на машине (кандидаты)
python3 scripts/consumers.py discover --apply               # принять кандидатов
python3 scripts/consumers.py check                          # дальше — обычный порядок
```

`init` пишет `catalog` (имя каталога и `owner/repo`), `machine`, `discoverRoots` и
**пустой** `consumers[]`. Пустой список — не заглушка и не «мы не знаем, кого
обслуживаем»: это честная запись «ещё ничего не принято», и `check` сразу называет
кандидатом каждого настоящего потребителя. Список наполняет `discover --apply` из
наблюдения, потому что потребителей знает машина, а не память оператора — ровно тот
довод, с которого этот реестр и заведён (§«Зачем он есть» выше).

Почему кандидатов не принимает сам `init`:

- **`.omp/` — не доказательство потребительства.** У репозитория-плагина `.omp/` это
  **сам пакет** (ADR §5), и такой каталог потребителем не является; признак —
  `.omp/package.json`, которого у установки нет. Кандидаты с этим признаком и с
  `.omp-plugin/marketplace.json` (каталог маркетплейса) молча не принимаются и
  сообщаются как «не принят»; осознанное исключение — назвать явно `--only <имя>`,
  обычное решение — `--exclude <путь>`.
- **Корни поиска — знание оператора.** Под одним корнем видны не все потребители,
  поэтому `--root` обязателен и может повторяться; молчаливого корня по умолчанию нет —
  он бы прятал потребителей, то есть ровно тот пробел, ради которого реестр заведён.

Ошибка «реестр не найден» (exit `2`) печатает путь восстановления, а не только
констатацию: подсказка содержит команду `init`, запускаемую из любого каталога.

## Формат

Верхний уровень (`schemas/consumer-registry.schema.json`, draft 2020-12):

| Поле | Смысл |
|---|---|
| `schemaVersion` | версия формата; инструмент отказывается читать незнакомую |
| `machine` | hostname — видно, что реестр описывает одну машину |
| `updated` | когда реестр последний раз писала машина |
| `catalog` | `name` + `remote` каталога, к которому реестр относится |
| `discoverRoots` | каталоги, в которых ищутся проекты с `.omp/` (обнаружение неучтённых) |
| `exclude` | пути, которые не потребители: репозитории-источники плагинов, машинный слой |
| `consumers` | сам список; `name` и `path` уникальны |

Потребитель:

| Поле | Смысл |
|---|---|
| `name` | слаг для `--only` |
| `path` | абсолютный путь к корню проекта |
| `plugins[]` | **ожидаемый состав**: `id` (`<плагин>@<каталог>`), `scope` (`project`/`user`), `installed` (наблюдаемое, `null` — не поставлен), `pin` (проект сознательно держит версию) |
| `legacy.flat[]` | файлы под `.omp/<dir>`, требующие внимания: перекрывающие поставленный пакет и неразличимые без эталона (пакет не поставлен), путями от корня проекта |
| `worktreeInit` | `hook` (путь к скрипту инициализации ворктри) + `installsPlugins` (ставит ли он плагины по-настоящему) |
| `notes` | заметка оператора: особенности, известные отклонения |
| `lastChecked`, `lastUpgraded` | когда инструмент наблюдал / обновлял |

**Желаемой версии в реестре нет намеренно.** Версию для `omp plugin upgrade` задаёт
запись каталога (см. [контракт каталога](marketplace-catalog.md)); реестр хранит только
наблюдаемое и ожидаемый *состав*. Две правды о версии — тот самый дрейф, от которого
уходим.

`pin` — единственное исключение и оно явное: проект **сознательно** держит версию.
Пин, который соблюдён, но отстал от каталога, — `pinned` (не ошибка). Пин, который
нарушен (стоит не то, что закреплено), — уже `stale`: обещание в реестре разошлось с
машиной.

## Откуда берётся «актуальная» версия

Инструмент читает **кэш каталога** — тот же файл, из которого работает `omp plugin
upgrade`: `~/.omp/marketplaces.json` → `catalogPath` нужного маркетплейса. Рабочее
дерево этого репо не читается: локальный `main` может быть впереди запушенного, и
тогда `check` показывал бы дрейф, который потребитель физически не может закрыть до
push. Обновление кэша — отдельный явный шаг (`upgrade` делает его сам, `check`
принимает `--refresh`).

## Что считается дрейфом

| Вид | Признак | `upgrade` |
|---|---|---|
| `missing` | в реестре есть, `installed: null` | ставит плагин |
| `stale` | `installed` ≠ версия в кэше каталога, либо пин нарушен (в том числе «закреплён, а не поставлен») | обновляет (кроме закреплённых) |
| `scope` | установлен в `user`, а в реестре `project` | ставит в `project` (user-scope виден во всех проектах и путает версии) |
| `materialization` | реестр установки говорит «плагин есть», а payload недоступен: мёртвый `installPath` (кэш снесён), нет записи в `node_modules`, ведущей на него, или битый симлинк | ничего: перепривязывает `omp plugin upgrade <id>@<каталог> --scope=…` в затронутом проекте — то же лечение, что в [runbook](../ops/upgrade-consumers.md) §6 |
| `unknown` | плагин в реестре есть, а каталог его не знает (опечатка в id, незарегистрированный маркетплейс, пустой кэш) | ничего: ставить нечего, причина печатается |
| `unversioned` | каталог плагин знает, а версии не объявил | ничего: обновлять нечего, **не дрейф** — в отчёте помечается `•` |
| `pinned` | `pin` соблюдён, но каталог ушёл вперёд | **пропускает**, показывает как `pinned` (это не ошибка) |
| `legacy` | под `.omp/<dir>` есть файлы, чьи пути есть в поставленном пакете — они перекрывают плагин (приоритет 100 против 90) | ставит плагин, но копии надо снять: `migrate` |
| `legacy-unverified` | под `.omp/<dir>` есть файлы, а поставленных пакетов нет — сравнивать не с чем | ставит плагин; различение «копия или своё» станет возможным после `upgrade` |
| `own-rules` | под `.omp/<dir>` есть файлы, которых нет ни в одном пакете | ничего: это собственные файлы проекта под неплагинными именами (ADR §6), **не дрейф** — в отчёте помечается `•` |
| `worktree-gap` | хук ворктри есть, но плагины в них не ставит | ничего — правится хуком проекта; ворктри останутся без плагинов (ADR §4) |
| `unmanaged` | проект с `.omp/` не в реестре и не в `exclude` | ничего; сообщается как кандидат в реестр |
| `gitignore` | проект не игнорирует доставленные/производные пути — они попадут в git | ничего (как `worktree-gap`): дополнить `.gitignore` каноническим блоком из [consumer-repo-layout](consumer-repo-layout.md); detail перечисляет непокрытые строки |
| `error` | потребитель недоступен: пути нет, `.omp/` нет, `omp plugin list` провалился или вернул не-JSON | ничего; строка отчёта, остальные потребители проверяются дальше |

`error` — не вид дрейфа, а отказ наблюдения: он не должен ронять отчёт по остальным
потребителям, но код выхода остаётся `1`. Исключение — отсутствие самого `omp` в PATH:
это отказ окружения (exit `2`), а не дрейф: без `omp` инструменту нечего наблюдать.

Материализация проверяется в **обоих корнях установки**: проектный плагин линкуется в
`<проект>/.omp/plugins/node_modules`, машинный — в `~/.omp/plugins/node_modules` (кэш
пакетов у omp при этом общий). Записи сопоставляются с плагинами **по цели симлинка**,
а не по имени: имени пакета в `omp plugin list --json` нет, и оно не обязано совпадать
с именем плагина (`1c` ставится пакетом `1c-omp`). Одна поломка — одна строка: если
цель битого симлинка и есть `installPath` плагина, называет её строка симлинка, а не
вторая строка про тот же путь.

Мягкие виды — `pinned`, `own-rules`, `unversioned`: они не делают гейт красным, и
перечень один на весь инструмент. `upgrade` для `unversioned` печатает пропуск с `•`
и остаётся зелёным — обновлять действительно нечего; жёсткий пропуск (`unknown`,
недоступный потребитель) по-прежнему даёт exit `1`.

Покрытие `gitignore`: шаблон закрывает требуемый путь, если накрывает сам путь или
любой его каталог-предок; `*` и `?` не переходят через `/`, `**` переходит — как их
читает git. Ведущий `/` (привязка к корню) и хвостовой слэш на смысл не влияют.
Отмена (`!`) в проверке **не участвует**: она делает путь версионируемым, а это
решение проекта о своём — свои `rules/skills/commands` по политике живут в git.
Набор требуемых строк — канонический блок
[consumer-repo-layout](consumer-repo-layout.md); тест сверяет константу кода с этим
блоком, поэтому политика и код не расходятся молча.

## Контракт инструмента

```
python3 scripts/consumers.py init     --root DIR [--root DIR]... [--exclude PATH]... [--json]   # заведение реестра; единственная команда без реестра
python3 scripts/consumers.py discover [--json]                       # что есть на машине; кандидаты
python3 scripts/consumers.py discover --apply [--only NAME]... [--exclude PATH]...   # принять кандидатов
python3 scripts/consumers.py scan     [--json]          # обновить наблюдаемое: installed, legacy.flat, worktreeInit
python3 scripts/consumers.py check    [--refresh] [--only NAME] [--json]
python3 scripts/consumers.py upgrade  [--only NAME] [--dry-run] [--yes] [--json]
python3 scripts/consumers.py verify   [--only NAME] [--json]   # gitmark index + deploy-check.sh + gitmark version
python3 scripts/consumers.py migrate  [--only NAME] [--apply] [--keep PATH] [--accept-divergent] [--accept-unique] [--json]   # снос плоских копий
```

- Коды выхода: `0` — чисто; `1` — найден дрейф, недоступный потребитель или провал
  проверки; `2` — ошибка использования либо реестр не соответствует схеме.
- `init` — единственная команда, работающая **без** реестра, и единственная, которая
  его создаёт. Требует хотя бы один `--root` и отказывается работать, если реестр уже
  есть: перезапись реестра — не то, что делают по ошибке.
- `discover` без `--apply` — только отчёт, как `migrate` без `--apply`. `--only` и
  `--exclude` — **модификаторы записи**: без `--apply` они дают exit `2`, чтобы в
  инструменте не было флага, который ничего не меняет. Исключения применяются **до**
  принятия, поэтому названный не-потребителем путь не попадает в `consumers` даже
  транзитом. Повторный `discover --apply` ничего не дублирует.
- Принятие записывает ожидаемый состав = наблюдаемый: `installed` — фактическая
  версия, `pin: null`, `scope: "project"`. `scope` именно `project`, а не фактический:
  машинная (`user`) установка — это дрейф `scope`, и принятие его не благословляет.
  Исключение — плагины, версии которых каталог не объявил: в состав они **не** вносятся,
  а печатается строка «не отслеживается: каталог не объявляет версию». Иначе принятие
  сразу краснит гейт (`unversioned`), а обновлять такие плагины всё равно нечем.
  Плагин, которого каталог **не знает**, наоборот, вносится: это `unknown`, и он должен
  быть виден в `check` — опечатку в id или незарегистрированный маркетплейс чинят.
- `migrate` **без** `--apply` — только отчёт; это и есть его режим по умолчанию
  (отдельного `--dry-run` нет, чтобы не было флага, который ничего не меняет).
- Недоступный потребитель виден во всех командах и везде даёт exit `1`: в `check` —
  строкой `error`, в `upgrade` — записью в `skipped` (и в JSON, и в тексте), в
  `migrate` — записью в `errors`. `upgrade` не бывает зелёным, если хоть один
  потребитель не наблюдался.
- Глобальные ключи: `--registry` (по умолчанию `.consumers.json` в корне клона),
  `--schema`, `--marketplaces` (реестр omp — источник версий каталога). Последние два
  нужны тестам и нестандартным установкам; в обычной работе не требуются.
- Валидация реестра — **своим** валидатором, без внешних зависимостей: python-модуля
  `jsonschema` на машине нет (проверено 2026-09-14), а инструмент обязан запускаться на
  голом python3.
- `migrate` без `--apply` — только отчёт (что будет снято, что расходится с пакетом).
  Молча не удаляется ничего: файл, отличающийся от пакетного, — решение оператора.
- Классификация файла под `.omp/{skills,rules,commands,scripts}`: `identical` (совпал с
  поставленным пакетом), `divergent` (такой путь в пакете есть, содержимое иное),
  `unique` (пути в пакетах нет — **свой файл проекта**), `unverifiable` (пакетов нет
  вовсе — сравнивать не с чем).
- **Копии и свои файлы — разные вещи.** `identical` и `divergent` перекрывают плагин
  (нативный провайдер, приоритет 100) и подлежат снятию; `unique` — собственное знание
  проекта, которое по ADR §6 живёт здесь под именем, не совпадающим с плагинным.
  Поэтому `migrate --apply` снимает копии и **оставляет** свои файлы; блокируют его
  только расходящиеся копии (`--accept-divergent` — осознанное решение оператора) и
  отсутствие эталона (`unverifiable`, лечится `upgrade`). Свои файлы снимаются лишь по
  отдельному решению — `--accept-unique`. `--keep` сильнее любого `--accept*` и
  принимает и относительный путь, и абсолютный внутри проекта.
- Плоской копией считается каталог `.omp/{skills,rules,commands,scripts}` **с хотя бы
  одним файлом**: пустой каталог ничего не перекрывает и дрейфом не считается.
- `verify` различает уровень проверки: `gitmark index` и `version` — 0 или провал;
  `deploy-check.sh` — 0 чисто, 1 критика (провал), **2 предупреждения** (пакет
  работает: так `deploy-check` и документирован в шапке скрипта). В отчёте
  предупреждение помечается `⚠`, код выхода при нём остаётся `0`.
- `scan --json` печатает реестр с обновлённым наблюдаемым, но **не пишет** файл: так
  удобно сравнить до и после, не трогая реестр.
- **Свой dry-run, а не флаг omp.** `omp plugin upgrade --dry-run` флаг игнорирует и
  выполняет обновление (проверено 2026-09-14: `erp-demo`, `1c` 0.1.1 → 0.1.2 при
  `--dry-run`); у `install` та же болезнь — флаг разбирается, но обработчик установки
  его не читает (проверено 2026-09-16, omp v18.2.0: плагин действительно поставлен при
  `--dry-run`). Поэтому `--dry-run` инструмента обязан **не вызывать** `omp plugin
  upgrade` вовсе — только печатать план; проверять флагом omp нельзя.
- Инструмент идемпотентен: повторный `upgrade`/`scan` не меняет состояние.

## Поток поставки

```mermaid
graph LR
  R["релиз плагина<br/>тег + push"] --> C["bump записи каталога<br/>version + ref"]
  C --> P["push каталога"]
  P --> U["marketplace update<br/>(кэш каталога)"]
  U --> K["consumers check"]
  K --> G["consumers upgrade"]
  G --> V["consumers verify"]
```

`check` до `upgrade` — не формальность: он показывает, кого обновление затронет и где
стоит `pin`, до того как что-то изменилось.

## Состояние машины на 2026-09-14

> **Исторический снимок.** Реестр машино-локален, и в этом клоне его нет (свежий клон,
> 2026-09-16). Цифры ниже — на момент, когда контур заводили: по ним видно, зачем он
> нужен. Актуальное даёт `python3 scripts/consumers.py check` после восстановления
> реестра (§«Реестр на новой машине» выше).

Девять потребителей: `erp-demo` и `sot-omp-marketplace` — чистая плагинная установка,
`erp-main`, `erp-mini`, `ut-10`, `retail`, `zupupr`, `rusbread-master-site`,
`subvost-xray-tun` — с плоскими копиями (в `erp-main` и `zupupr` плагин стоит поверх
копий). Живая картина — `python3 scripts/consumers.py check`, а не этот абзац; он
фиксирует, почему контур заведён.

Работы по приведению машины к плагинной поставке — план
[consumer-delivery](../plans/consumer-delivery.md).
