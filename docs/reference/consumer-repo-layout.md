...
перечисляет непокрытые строки (в `--json` — полем `ignores`). Сверяется смысл, а не
символы: шаблон покрывает требуемую строку, если совпадает с ней или является
каталогом-префиксом. Поэтому огульное `.omp/` закрывает все строки под `.omp/`,
`/.gitmark/` и `.gitmark` равны `.gitmark/`, а отмена (`!`) покрытие снимает — она
возвращает путь в git ровно так же, как отсутствие строки.
